...
namespace AutoHelp.domain.Helpers.XMLHelper
{
    ...
    public sealed class XmlDocCommentReaderSettings : ConfigurationSection
    {
        ...
        public XmlDocCommentDirectoryElementCollection DirectoryNames =>
            (XmlDocCommentDirectoryElementCollection)this["XmlDocCommentDirectories"];
        ...
        private static XmlDocCommentReaderSettings CreateDefaultSettings()
        {
            ...
            var referenceAssembliesSubDirectoryName =
                Path.Combine("Reference Assemblies", Path.Combine("Microsoft", framework));
            var programFilesDirectoryName = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);

            // ReSharper disable once AssignNullToNotNullAttribute
            var netfxDirectoryName =
                Path.Combine(Path.GetDirectoryName(Environment.SystemDirectory), netfxSubDirectoryName);
            var referenceAssembliesDirectoryNameX86 =
                Path.Combine(programFilesDirectoryName, referenceAssembliesSubDirectoryName);
            var referenceAssembliesDirectoryName = Path.Combine(programFilesDirectoryName + " (x86)",
                referenceAssembliesSubDirectoryName);
